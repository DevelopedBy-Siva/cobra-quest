...
export const createPlayerElements = (player) => {
  if (!LOAD_DATA) {
    ...
    scoreElement.appendChild(
      document.createTextNode(`Highscore: ${player.score}`)
    );
    ...
    removeIcon.className = "fa fa-ban";
    ...
    listTag.addEventListener("click", () => {
      ...
      tags.forEach((i) => {
        ...
        if (active && name === active.name) i.classList.add("user-selected");
        else i.classList.remove("user-selected");
      });
      createCurrentPlayer();
    });
  }
...
function removePlayer(player) {
  ...
  if (allPlayers.getAllPlayers().length === 0) noPlayerMessage();
}
...
export function loadPlayers() {
  ...
  try {
    ...
    playerList.removeChild(
      playerList.getElementsByClassName("loading-spinner-wrap")[0]
    );
...
function noPlayerMessage() {
  ...
  element.innerText = "No player found";
...
function createCurrentPlayer() {
  try {
    ...
    document
      .getElementsByClassName("player-sub-wrapper")[0]
      .appendChild(playerElement);

    // Current Player Score
    const scoreElement = document.createElement("span");
    scoreElement.className = "current-user-score";
    scoreElement.innerHTML = `<span id="current-user-score-title">Score: ${getPlayer.score}</span>`;

    document
      .getElementsByClassName("player-wrapper")[0]
